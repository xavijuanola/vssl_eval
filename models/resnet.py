...
class ResNetSpec(nn.Module):
    ...
    def forward(self, x):
        ...
        #     D = librosa.amplitude_to_db(np.abs(librosa.stft(spec.detach().cpu().numpy(), hop_length=hop_length)),
        #                                 ref=np.max)
        #     librosa.display.specshow(D, y_axis='log', sr=16000, hop_length=hop_length,
        #                             x_axis='time', ax=ax)
        #     ax.set(title=f"{filename}")
        #     ax.label_outer()
        #     fig.savefig(f"/upf/ongoing/rcgrad_scratch/{filename}.png")
        #     plt.close()
