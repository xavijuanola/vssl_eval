...
def normalize_thresholded_img(value, vmax=None, vmin=None):
    # Create a mask for non-zero values
    non_zero_mask = value != 0
    
    # If there are no non-zero values, return the original array
    if np.any(non_zero_mask):
        # Calculate min and max only for non-zero values
        vmin = value[non_zero_mask].min() if vmin is None else vmin
        vmax = value[non_zero_mask].max() if vmax is None else vmax

        # Avoid division by zero if vmin == vmax
        if vmax > vmin:
            # Normalize only non-zero values
            value[non_zero_mask] = (value[non_zero_mask] - vmin) / (vmax - vmin)
